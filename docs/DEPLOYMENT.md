...
| 12 | Static assets are cached by browsers for 1 year (`immutable`) keyed only by a hand-maintained `?v=N` query string, not a content hash | **Confirmed, not just theoretical:** reproduced during the 2026-09-22 fixes ([change plan](changes/2026-09-22-secretary-feedback.md)) — a browser that already loaded the app kept running JS from *before* a fix, across hard reloads, until `?v=6` was manually bumped to `?v=7` everywhere. Any deploy that changes frontend JS/CSS without bumping the version string silently fails to reach users who already visited the site | Bump `?v=N` on every frontend change (done for 2026-09-22); longer term, automate with a build-time content hash instead of a manually incremented number |
